...
# Job submission should be done from the gen_babymaker directory, which is under a valid CMSSW release
codedir = os.getcwd()
if not ("/src/gen_babymaker") in codedir:
  print "\033[91mERROR: Please submit from path consistent with: .../src/gen_babymaker/ \033[0m\n"
  sys.exit(0)
...
if (host=="sd"): 
  print "INFO: Creating gen_babymaker tarball to transfer to work node..."
  ...
  tarcmd += " --exclude=\".git\""
  tarcmd += " -c gen_babymaker | xz > ../gen_babymaker.tar.xz"
  os.system(tarcmd)
...
for ids, ds in enumerate(sorted(files_dict.keys())):
  ...
  for job in range(0,njobs):
    ...
    if ("Run2015D" in ds):
      ...
      condor_args.append("json=gen_babymaker/"+json_name)
    ...
    if (host=="sb"):
      ...
      fexe.write("eval `scramv1 runtime -sh`\n")
      fexe.write("export ORIGIN_USER="+os.getenv("USER")+"\n")
      fexe.write("tar -xf ../../gen_babymaker.tar.xz\n")
      fexe.write("cd gen_babymaker\n")
    ...
    if (host=="sb"):
      ...
      fcmd.write("transfer_input_files = ../gen_babymaker.tar.xz\n")
      fcmd.write("Notification = Never\n")
      fcmd.write("Log          = "+logdir+"/"+bname+".log\n")
      fcmd.write("output       = "+logdir+"/"+bname+".out\n")
      fcmd.write("error        = "+logdir+"/"+bname+".err\n")
